...
void FrequencyModifiedFourierTransform P(( int _tp1));
...
#endif
{
	...
	FrequencyModifiedFourierTransform(_tp1);
...
static struct func {
	...
	const char  *f_name;
		...
		{ 1, 2, _tr0, "FrequencyModifiedFourierTransform" }
		};

static const char* evalstrs[] = {
	"FrequencyModifiedFourierTransform::usage=\"FrequencyModifiedFouri",
	"erTransform[n,{{t,x,y},...}] returns n frequencies from the FMFT",
	" of the data given\"",
	(const char*)0,
	"FrequencyModifiedFourierTransform::OddNFreq=\"`` is odd.  The FMF",
	"T is currently set to require an even number of frequencies only",
	"\"",
	(const char*)0,
	"FrequencyModifiedFourierTransform::FMFTFailure=\"The FMFT routine",
	" returned an error value\"",
	(const char*)0,
	"FrequencyModifiedFourierTransform::BadDimensions=\"The FMFT routi",
	"ne recieved an input with improper dimensions\"",
	(const char*)0,
	"FrequencyModifiedFourierTransform::mlink=\"MathLink failed with e",
	"rror message: ``\"",
	(const char*)0,
	(const char*)0
};
...
#endif
{
	int _res;
	_res = MLConnect(mlp);
	if (_res) _res = _definepattern(mlp, (char *)"FrequencyModifiedFourierTransform[n_Integer,data_?(VectorQ[#, (MatchQ[#, {Repeated[_?NumberQ, {3}]}] &)]&) ]", (char *)"{n,data}", 0);
